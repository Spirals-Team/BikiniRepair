...
import fr.inria.lille.spirals.bikinirepair.server.ip.AddressServer;
import fr.inria.lille.spirals.bikinirepair.server.ip.AddressServerImpl;
...
public class Services {
	...
	public static Remote getRegression() {
		try {
			Registry registry = LocateRegistry.getRegistry(localIp, registryPort);
			return registry.lookup("Regression");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public static Registry createRegressionServer(Remote remote) {
		return createServer(remote, "Regression");
	}
