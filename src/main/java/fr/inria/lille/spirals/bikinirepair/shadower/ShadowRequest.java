...
public class ShadowRequest {
	...
	public String getTarget(ServicesType target) {
		String host = "http://";
		try {
			switch (target) {
			case APPLICATION:
				host += fr.inria.lille.spirals.bikinirepair.services.Services.getAddressServer().getApplicationIP();
				break;
			case VALIDATION:
				host += fr.inria.lille.spirals.bikinirepair.services.Services.getAddressServer().getRegressionIP();
				break;
			case PATCH:
				host += fr.inria.lille.spirals.bikinirepair.services.Services.getAddressServer().getPatchIp();
				break;
			default:
				return null;
			}
	...
	public ShadowResponse execute(ServicesType target, String cookies) throws Exception {
